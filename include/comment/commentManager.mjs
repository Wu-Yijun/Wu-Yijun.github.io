...
const DefaultConsts = {
  ...
  UserHomeUrl: '/UserHome',
  log: `<img src="/include/comment/assets/Login.svg" height="20px">
        登录
        <img src="/include/comment/assets/GitHubR.svg" height="15px">
        <img src="/include/comment/assets/GithubIcon.svg" height="25px">`,
  user:
      ` <img class="Github-Login-User-Icon" src="/include/comment/assets/GithubIcon.svg" height="25px">
...
class GitControl {
    ...
    try {
      if (location.search) {
        ...
        history.replaceState(
            {code: code, search: location.search}, '', this.oauthInfo.Url);
      }
    } catch (e) {
      console.log('Error: ', e);
    }
  ...
  static REQUEST(type = 'POST', url, header, data, resposeFun, retried = 3) {
    ...
    if (type === 'GET' && data)
      request.open(
          type, url + '?' + (new URLSearchParams(data)).toString(), true);
    else
  ...
  showInfo() {
    ...
    if (typeof location.search == 'string' && location.search.length > 0) {
      /** this will not reload pages. */
      history.replaceState(
          {code: -1, search: location.search}, '', this.oauthInfo.Url);
    }
    Array.prototype.forEach.call(
        document.getElementsByClassName(this.oauthInfo.GitLoginClass),
        (elem) => {
      ...
      GitControl.REQUEST('GET', url, header, data, (request) => {
        try {
          if (request.status != 200) {
            myFloatingNotify(
                'Network error:' + JSON.parse(request.responseText).message,
                5000);
  ...
  constructor(config) {
    ...
    setACTION(document, 'comment-textarea', (container) => {
      ...
      setACTION(cc, 'comment-textarea-texts', (txt) => {
        txt.onkeydown =
            (key) => {
              if ((key.isTrusted && key.code == 'Enter' && key.ctrlKey) &&
                  !key.metaKey && !key.repeat && !key.shiftKey && !key.altKey) {
                toSubmit();
              }
            }
      })
...
export default class commentManager {
  ...
  onload() {
    if (this.config.hash) {
      ...
      if (hashs.length > 0) location.hash = hashs[0];
    }
  }
  ...
  async getCommentNum() {
    ...
    if (num > this.commentsNum) this.commentsNum = num;
  ...
  getDom(index) {
    if (index instanceof HTMLElement) return index;
    if (index < 0) index = 0;
    if (index >= this.comments.length) index = this.comments.length - 1;
