...
我先排除了 WebAssembly GUI ，与其写成 WebAssembly GUI ，还不如我直接用 Javascript 做成网页，反正又没什么性能的追求。（但是 Tauri 相比其它的几个库，的确成熟很多，但因为是 Webview 我不想用）
...
目前我找到的两个感觉还可以的库分别为:
! [fltk-rs](https://fltk-rs.github.io/fltk-rs/) ! [@Github](https://github.com/fltk-rs)
! [gtk4-rs](https://gtk-rs.org/gtk4-rs/stable/latest/book/) ! [@Github](https://github.com/gtk-rs/gtk4-rs)

我先花了两天把 fltk 大体搞清楚了。它的确是一个挺不错的库，全部的 UI 都是通过点线绘制出来的，然后也提供了几个默认主题，简单的可以通过主题加颜色样式等来调整外观，稍复杂的可以通过向 draw 内传入自定的绘制函数来自主控制，如果更复杂的绘图可以使用 OpenGl 窗口完成绘制。
fltk 的优势在于，因为图标是它绘制的，因此它几乎没有平台依赖性，可以方便的移植到 Android/Mac/Linux/Windows 等操作系统。并且程序体积很小，编译速度快，打开时加载时间短。
但是，fltk 界面很丑陋，而且对于颜色的支持不佳，更不用提纹理这种复杂的。你甚至连一个渐变色的矩形都要通过线条组合起来。然后它不支持透明度，元素的层次关系也不好，比如，我想要在一个 opengl 界面上方叠加一个按钮，一方面两者的位置关系理不清，一方面按钮还不能半透明，就很丑。还有，你要使用Opengl就会触发 unsafe ，这也不好解决.

然后我又看了 gtk4 ，这个库是 C++ 的库，然后链接入 Rust 的。它的历史比较悠久，因此东西很多（至少是C++）。
它的界面相比于 fltk 更好看，代码层面大同小异，稳定性比 fltk 强。编译的 release 版本大小仅 10MB ，相比 gtk4 的 30MB 小不少。
但是它体积很大，编译速度慢，启动时间长，没法交叉编译到 Android 平台，这是缺点。从目前看到的教程来说，我其实更喜欢 gtk4 的。但我在网上发现，它似乎因为历史因素，有些过于臃肿繁琐了，不适合入手。

接下来后我准备测试一下轻量化的 eframe + egui
它的编译构建速度很快，编译好的大小仅 11 MB ，启动速度很不错。
> egui是Rust原生的即时模式GUI图形库。采用即时模式意味着每一帧重新绘制所有界面。也就是说在一般的静态布局界面的应用下，这种模式的性能开销是更大的。但是在一些界面变化较多较快的场景（例如一个简单的贪吃蛇小游），它的性能开销会小于使用保持模式的GUI应用。 

我觉得可以先试试水，如果 egui 不好我再切回 gtk4(大保底)
...
程序草稿包含两个内容，一个是界面切换的逻辑，另一个是游戏关卡渲染的逻辑。

#### 界面逻辑。

我不打算搞的多么高级，那太复杂了，笨孩子可以用粗暴的蠢办法。
在渲染循环内，我们直接 if else 切换到对应的页面即可。

后面的内容我就直接写在 ! [@repo](Cube-Infinifold) 中了，只有到最后整合的时候我再将生成的网页放过来。